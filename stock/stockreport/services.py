...
class LiveSQLService:
    ...
    def get_item_param_stock_live(self, item_id=None, bcn=None, search=None):
        """
        Fetch item parameter details from SQL Server (itemParamDet table with parameter columns).
        Args:
            item_id: Optional item code to filter
            bcn: Optional BCN to filter
            search: Optional search string for item name, BCN, or parameters
        Returns:
            List of dicts with item parameter details
        """
        with self.get_connection() as conn:
            cursor = conn.cursor()
            # Join itemParamDet with Master1 to get item name
            query = '''
                SELECT t.Date, t.VchType, t.VchNo, t.ItemCode, m.Name, t.C1, t.C2, t.C3, t.C4, t.C5, t.D3, t.D4, t.BCN, t.Value1
                FROM itemParamDet t
                JOIN Master1 m ON t.ItemCode = m.Code
                WHERE m.MasterType = 6
            '''
            params = []
            if item_id:
                query += ' AND t.ItemCode = ?'
            ...
            if search:
                query += ' AND ('
                query += 'm.Name LIKE ? OR t.BCN LIKE ? OR t.C1 LIKE ? OR t.C2 LIKE ? OR t.C3 LIKE ? OR t.C4 LIKE ? OR t.C5 LIKE ?'
                query += ')'
                search_param = f"%{search}%"
                params.extend([search_param]*7)
            query += ' ORDER BY m.Name, t.BCN, t.Date'
